...
RULES = [
    ...
    (
        ...
        re.compile(
            ...
            r"|не\s+рассматриваем|откажемся|спасибо,\s*нет",
            re.IGNORECASE,
        ),
    ),
    # «Уже есть / работаем с …» — не отказ, а повод спросить, чего не хватает
    # (s-rocket.com, vc.ru 2026).
    (
        "already_have",
        re.compile(
            r"уже\s+(есть|внедрили|работаем|пользуемся|стоит)|у\s+нас\s+(есть|свой|своя)\s"
            r"|работаем\s+с\s+(другим|другой|\w+ом\b)",
            re.IGNORECASE,
        ),
    ),
    (
        "no_time",
        re.compile(
            r"нет\s+времени|некогда|сейчас\s+не\s+до|занят[аы]?\b|позже\s+напиш|не\s+сейчас",
            re.IGNORECASE,
        ),
    ),
    (
        "think",
        re.compile(
            r"подума(ем|ю)|посоветуемся|обсудим\s+(внутри|с\s+коллег)|надо\s+обсудить",
            re.IGNORECASE,
        ),
    ),
    (
...
ACTION = {  # категория → действие из policy.yaml
    ...
    "already_have": "reply_already_have",
    "no_time": "reply_no_time",
    "think": "reply_think",
...
TEMPLATES = {
    ...
    "already_have": (
        "{Hi}понял, значит тема вам знакома. А есть что-то, что в текущей схеме "
        "всё равно делают руками?"
    ),
    "no_time": "{Hi}понимаю. Давайте я напишу через пару недель, так будет удобнее?",
    "think": (
        "{Hi}конечно. Если поможет, пришлю в двух строках, что сказать коллегам, "
